...
// RunActor represents the type for RunActor
// The category of the principal that created the run, determined by Scalr from the request credentials and client headers. The identity itself is the `created-by` relationship.
type RunActor string

// RunActor constants
const (
	RunActorUnknown        RunActor = "unknown"
	RunActorUser           RunActor = "user"
	RunActorServiceAccount RunActor = "service-account"
	RunActorAiAgent        RunActor = "ai-agent"
	RunActorScalrSystem    RunActor = "scalr-system"
)
...
type RunAttributes struct {
	...
	// The IaC platform for the run.
	IacPlatform RunIacPlatform `json:"iac-platform"`
